...
class Cadastro {
	...
	function coletaDados($id,$tipo) {
		...
		switch ($tipo) {
			...
			break;			
               		...
               		case 8:
				$this->DB->selectTab("contratos c, cad_clientes cl, cad_instaladores i, os o", "c.*, cl.*,i.*, o.*", "WHERE i.id=o.id_instalador AND c.id=o.id_contrato AND c.id_cliente = cl.id AND o.id = '$id'");
			break;			
	...
	function cadastrarOS($REQUEST,$tipo) {
		...
		switch ($tipo) {
			case "1": // UPLOAD
				...
				// Se cadastrou registra no LOG a acao
				$this->LOG->insertLog($this->DB,'O usuário "'.$this->login.'" ENVIOU Arquivo [ '.$file.' ].');
	...
	function adicionarModulosInstalador($instalador,$modulos) {
		...
		$this->DB->updateTab("cad_equipamentos", "status = 4", "id IN ($modulos)");
