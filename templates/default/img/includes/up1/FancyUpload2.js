...
var FancyUpload2 = new Class({
	...
	onLoad: function() {
		this.log('Upload pronto!');
	},
 
	onBeforeOpen: function(file, options) {
		this.log('Iniciar upload de "{name}".', file);
	...
	onOpen: function(file, overall) {
		this.log('Iniciando upload "{name}".', file);
		file = this.getFile(file);
		file.element.addClass('file-uploading');
		this.currentProgress.cancel().set(0);
		this.currentTitle.set('html', 'Progresso do arquivo "{name}"'.substitute(file) );
	},
 
	onProgress: function(file, current, overall) {
		this.overallProgress.start(overall.bytesLoaded, overall.bytesTotal);
		this.currentText.set('html', 'Enviando com {rate}/s. Tempo estimado: ~{timeLeft}'.substitute({
	...
	onAllComplete: function(current) {
		this.log('Completo todos os arquivos, ' + current.bytesTotal + ' bytes.', arguments);
		this.updateOverall(current.bytesTotal);
	...
	upload: function(options) {
		...
		if (ret !== true) {
			this.log('Upload em andamento ou nada para carregar.');
			if (ret) alert(ret);
		} else {
			this.log('Upload iniciado.');
	...
	updateOverall: function(bytesTotal) {
		...
		this.overallTitle.set('html', 'Progresso Total (' + this.sizeToKB(bytesTotal) + ')');
	},
