-- showtecsystem.cad_bancos definition

CREATE TABLE `cad_bancos` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Identificador dos registros',
  `codigo` varchar(10) NOT NULL COMMENT 'Codigo do banco',
  `nome` varchar(100) NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=307 DEFAULT CHARSET=utf8mb4;


INSERT INTO showtecsystem.cad_bancos (codigo, nome) VALUES
    ('001', 'Banco do Brasil S.A.'),
    ('003', 'Banco da Amazônia'),
    ('004', 'Banco do Nordeste do Brasil S.A.'),
    ('012', 'Banco Standard de Investimentos S.A.'),
    ('014', 'Natixis Brasil S.A. Banco Múltiplo'),
    ('019', 'Banco Azteca do Brasil'),
    ('021', 'BANESTES S.A. Banco do Estado do Espírito Santo'),
    ('024', 'Banco de Pernambuco S.A.'),
    ('025', 'Banco Alfa'),
    ('029', 'Banco Banerj'),
    ('031', 'Banco Beg'),
    ('033', 'Banco Santander (Brasil) S.A.'),
    ('036', 'Banco Bradesco BBI'),
    ('037', 'Banco do Estado do Pará S.A.'),
    ('039', 'Banco do Estado do Piauí S.A.'),
    ('040', 'Banco Cargill'),
    ('041', 'Banco do Estado do Rio Grande do Sul S.A.'),
    ('044', 'Banco BVA'),
    ('045', 'Banco Opportunity S.A.'),
    ('047', 'Banco do Estado de Sergipe S.A.'),
    ('062', 'Hipercard Banco Múltiplo S.A.'),
    ('063', 'Banco Ibi S.A. Banco Múltiplo'),
    ('064', 'Goldman Sachs do Brasil Banco Múltiplo S.A.'),
    ('065', 'Banco Bracce'),
    ('066', 'Banco Morgan Stanley S.A.'),
    ('069', 'BPN Brasil Banco Múltiplo S.A.'),
    ('070', 'BRB'),
    ('072', 'Banco Rural Mais S.A.'),
    ('073', 'BB Banco Popular do Brasil S.A.'),
    ('074', 'Banco J. Safra S.A.'),
    ('075', 'Banco CR2'),
    ('076', 'Banco KDB S.A.'),
    ('077', 'Banco Intermedium S.A.'),
    ('078', 'BES Investimento do Brasil'),
    ('079', 'JBS Banco S.A.'),
    ('081', 'Concórdia Banco S.A.'),
    ('082', 'Banco Topázio S.A.'),
    ('083', 'Banco da China Brasil'),
    ('084', 'Unicred Norte do'),
    ('085', 'Cooperativa Central de Crédito'),
    ('086', 'OBOE Crédito Financiamento e Investimento S.A.'),
    ('087', 'Cooperativa Unicred Central Santa Catarina'),
    ('088', 'Banco Randon S.A.'),
    ('089', 'Cooperativa de Crédito Rural da Região de Mogiana'),
    ('090', 'Cooperativa Central de Economia e Crédito Mutuo das Unicreds'),
    ('091', 'Unicred Central do Rio Grande do'),
    ('092', 'Brickell S.A. Crédito, financiamento e Investimento'),
    ('094', 'Banco Petra S.A.'),
    ('096', 'Banco BM&F de Serviços de Liquidação e Custódia'),
    ('097', 'Cooperativa Central de Crédito Noroeste Brasileiro Ltda.'),
    ('098', 'Credicorol Cooperativa de Crédito Rural'),
    ('099', 'Cooperativa Central de Economia e Credito Mutuo das Unicreds'),
    ('104', 'Caixa Econômica Federal'),
    ('107', 'Banco BBM'),
    ('133', 'Cresol Cooperativa'),
    ('168', 'HSBC Finance (Brasil) S.A.'),
    ('184', 'Banco Itaú BBA S.A.'),
    ('204', 'Banco Bradesco Cartões'),
    ('208', 'Banco BTG Pactual'),
    ('212', 'Banco Matone S.A.'),
    ('213', 'Banco Arbi'),
    ('214', 'Banco Dibens S.A.'),
    ('215', 'Banco Comercial e de Investimento Sudameris'),
    ('217', 'Banco John Deere S.A.'),
    ('218', 'Banco Bonsucesso'),
    ('222', 'Banco Credit Agricole Brasil'),
    ('224', 'Banco Fibra S.A.'),
    ('225', 'Banco Brascan'),
    ('229', 'Banco Cruzeiro do Sul'),
    ('230', 'Unicard Banco Múltiplo'),
    ('233', 'Banco GE Capital S.A.'),
    ('237', 'Banco Bradesco'),
    ('241', 'Banco Clássico'),
    ('243', 'Banco Máxima S.A.'),
    ('246', 'Banco ABC Brasil'),
    ('248', 'Banco Boavista Interatlântico'),
    ('249', 'Banco Investcred Unibanco S.A.'),
    ('250', 'Banco Schahin S.A.'),
    ('254', 'Paraná Banco S.A.'),
    ('260', 'NU PAGAMENTOS'),
    ('263', 'Banco Cacique'),
    ('265', 'Banco Fator S.A.'),
    ('266', 'Banco Cédula'),
    ('290', 'Pag Seguro'),
    ('300', 'Banco de La Nacion'),
    ('318', 'Banco BMG'),
    ('320', 'Banco Industrial e Comercial S.A.'),
    ('336', 'Banco C6'),
    ('341', 'Itaú Unibanco S.A.'),
    ('356', 'Banco Real S.A.'),
    ('366', 'Banco Société Générale Brasil S.A.'),
    ('370', 'Banco WestLB do Brasil S.A.'),
    ('376', 'Banco J. P. Morgan S.A.'),
    ('389', 'Banco Mercantil do Brasil S.A.'),
    ('394', 'Banco Bradesco Financiamentos'),
    ('399', 'HSBC Bank Brasil S.A.'),
    ('409', 'UNIBANCO'),
    ('412', 'Banco Capital S.A.'),
    ('422', 'Banco Safra S.A.'),
    ('453', 'Banco Rural S.A.'),
    ('456', 'Banco de'),
    ('464', 'Banco Sumitomo Mitsui Brasileiro S.A.'),
    ('473', 'Banco Caixa Geral'),
    ('477', 'Citibank N.A.'),
    ('479', 'Banco ItaúBank S.A'),
    ('487', 'Deutsche Bank S.A.'),
    ('488', 'JPMorgan Chase Bank'),
    ('492', 'ING Bank N.V.'),
    ('494', 'Banco de La Republica Oriental del'),
    ('495', 'Banco de La Provincia de Buenos'),
    ('505', 'Banco Credit Suisse (Brasil)'),
    ('600', 'Banco Luso Brasileiro S.A.'),
    ('604', 'Banco Industrial do Brasil S.A.'),
    ('610', 'Banco VR S.A.'),
    ('611', 'Banco Paulista S.A.'),
    ('612', 'Banco Guanabara S.A.'),
    ('613', 'Banco Pecúnia S.A.'),
    ('623', 'Banco Panamericano S.A.'),
    ('626', 'Banco Ficsa S.A.'),
    ('630', 'Banco Intercap S.A.'),
    ('633', 'Banco Rendimento S.A.'),
    ('634', 'Banco Triângulo S.A.'),
    ('637', 'Banco Sofisa S.A.'),
    ('638', 'Banco Prosper S.A.'),
    ('641', 'Banco Alvorada'),
    ('643', 'Banco Pine S.A.'),
    ('652', 'Itaú Unibanco Holding S.A.'),
    ('653', 'Banco Indusval S.A.'),
    ('654', 'Banco A.J.Renner'),
    ('655', 'Banco Votorantim S.A.'),
    ('707', 'Banco Daycoval'),
    ('719', 'Banif'),
    ('721', 'Banco Credibel'),
    ('724', 'Banco Porto Seguro S.A.'),
    ('734', 'Banco Gerdau S.A.'),
    ('735', 'Banco Pottencial S.A.'),
    ('738', 'Banco Morada S.A.'),
    ('739', 'Banco BGN'),
    ('740', 'Banco Barclays'),
    ('741', 'Banco Ribeirão Preto S.A.'),
    ('743', 'Banco Semear S.A.'),
    ('744', 'BankBoston N.A.'),
    ('745', 'Banco Citibank'),
    ('746', 'Banco Modal S.A.'),
    ('747', 'Banco Rabobank International Brasil S.A.'),
    ('748', 'Banco Cooperativo Sicredi'),
    ('749', 'Banco Simples S.A.'),
    ('751', 'Dresdner Bank Brasil S.A.'),
    ('752', 'Banco BNP Paribas Brasil'),
    ('753', 'NBC Bank Brasil S.A.'),
    ('755', 'Bank of America Merrill Lynch Banco Múltiplo S.A.'),
    ('756', 'Banco Cooperativo do Brasil'),
    ('757', 'Banco KEB do Brasil S.A.');
