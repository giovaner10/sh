function exportarArquivo(tipo, gridOptions, menu, titulo) {
    let colunas = [];
    if (menu === 'RelatorioTicketsShow') {
        colunas = ['id', 'cliente', 'situacao', 'nomeUsuario', 'usuario', 'departamento', 'assunto', 'ultimaInteracao', 'empresa', 'mensagemStatus', 'dataAbertura', 'dataFechamento', 'tempoGasto'];
    } else if (menu === 'RelatorioTicketsOmni') {
        colunas = ['id', 'cliente', 'situacao', 'nomeUsuario', 'usuario', 'departamento', 'assunto', 'ultimaInteracao', 'empresa', 'mensagemStatus', 'dataAbertura', 'dataFechamento', 'tempoGasto'];
    }
...
function printParams(pageSize) {
    return {
        ...
        PDF_LOGO: BaseURL + 'media/img/new_icons/shownet.png',
...
$(document).ready(function() {
    var dropdown = $('#opcoes_exportacao_omni');

    $('#dropdownMenuButtonOmni').click(function() {
        dropdown.toggle();
    });

    $(document).click(function(event) {
        if (!dropdown.is(event.target) && event.target.id !== 'dropdownMenuButtonOmni') {
            dropdown.hide();
        }
    });
});

function preencherExportacoesShow(gridOptions) {
    ...
    `;
        ...
        div.addEventListener('click', function(event) {
            ...
            exportarArquivo(opcao, gridOptions, 'RelatorioTicketsShow', 'Relatório Gerenciador de Tickets');
        });
...
function preencherExportacoesOmni(gridOptions) {
    const formularioExportacoes = document.getElementById('opcoes_exportacao_omni');
    const opcoes = ['csv', 'excel', 'pdf'];
    ...
    `;
        ...
        div.addEventListener('click', function(event) {
            event.preventDefault();
            exportarArquivo(opcao, gridOptions, 'RelatorioTicketsOmni', 'Relatório Gerenciador de Tickets');
        });
