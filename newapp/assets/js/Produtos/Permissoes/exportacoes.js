...
function exportarArquivo(tipo, gridOptions, titulo, tabela) {
    var colunas = [];
    if (tabela == 'Permissoes') {
        colunas = ["id", "descricao", "codPermissao", "modulo", "tecnologia", "status"];
    } else if (tabela == 'Produtos') {
        colunas = ["id", "nome", "descricao", "codigoProduto", "status"];
    }
...
$(document).ready(function() {
    var dropdown = $('#opcoes_exportacao_produtos');

    $('#dropdownMenuButtonProdutos').click(function() {
        dropdown.toggle();
    });

    $(document).click(function(event) {
        if (!dropdown.is(event.target) && event.target.id !== 'dropdownMenuButtonProdutos') {
            dropdown.hide();
        }
    });
});
...
function fecharDrop() {
    ...
    $('#opcoes_exportacao_produtos').hide();
}
...
function preencherExportacoes(gridOptions, titulo, tabela, botaoId) {
    const formularioExportacoes = document.getElementById(botaoId);
    ...
    `;
        ...
        div.addEventListener("click", function (event) {
            ...
            exportarArquivo(opcao, gridOptions, titulo, tabela);
        });
