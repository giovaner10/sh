...
const getListarRelatorioTecnicos = Router + "/listarTodosRelatorioServerSide";
...
$(document).ready(function () {
	...
	$("#menu-dashboard-instalacao").on("click", function () {
		if (!$(this).hasClass("selected")) {
			$(this).addClass("selected");
			$("#menu-relatorio-instalacao").removeClass("selected");
			$(".card-dados").hide();
			$(".card-dashboard").show();
			limparFiltros();
		}
	...
	$("#downloadChart")
		...
		.click(function (event) {
			...
			agCharts.AgCharts.download(chartBarModal, {
				width: 800,
				height: 500,
				fileName: "Relatório de Instalação - Gráfico Barra",
			});
		});
	...
	$("#downloadChartLine")
		...
		.click(function (event) {
			...
			agCharts.AgCharts.download(chartLineModal, {
				width: 800,
				height: 500,
				fileName: "Relatório de Instalação - Gráfico Linha",
			});
		});
...
function getDados(searchOptions) {
	if (searchOptions) {
		$.ajax({
			url: Router + "/listarRelatorioSmsPorNomeTecnicoEData",
			type: "POST",
			data: searchOptions,
			dataType: "json",
			beforeSend: function () {
				...
				if (data.status === 200) {
					...
					if ("mensagem" in data.results) {
						if (
							...
							showAlert('warning', data.results.mensagem);
						}
					} else {
						showAlert('error', 'Não foi possível fazer a listagem.')
					}
		...
		$.ajax({
			url: Router + "/listarUltimosCemRelatorioTecnicos",
			type: "GET",
			dataType: "json",
			beforeSend: function () {
				showLoadingChart();
...
function getServerSideReport(options, type = "pdf") {
	...
	axios
		.post(Router + "/generateServerSideReportInstalacao", requestData, {
			headers: {
