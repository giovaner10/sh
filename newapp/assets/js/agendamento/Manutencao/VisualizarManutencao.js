var AgGridSmsManutencao;
async function atualizarAgGridSmsManutencao(options) {
	stopAgGRIDSms();
	...
	function getServerSideData() {
		...
		return fetch(routePegarSMSManutencao, {
			method: "POST",
			cache: "no-cache",
			body: requestParams,
		})
	...
	const gridOptions = {
		columnDefs: [
			...
			{
				headerName: "Telefone Mantenedor",
	...
	AgGridSmsManutencao = new agGrid.Grid(gridDiv, gridOptions);
...
var AgGridAuditTrackManutencao;
async function atualizarAgGridAuditTrackManutencao(options) {
	stopAgGRIDAuditTrack();

	function getServerSideData() {
		let requestParams = new FormData();
		requestParams.append("idConversation", options.idConversation);
		requestParams.append("idMantenedor", options.idMantenedor);

		return fetch(routePegarAuditTrackManutencao, {
			method: "POST",
			cache: "no-cache",
			body: requestParams,
		})
	...
	AgGridAuditTrackManutencao = new agGrid.Grid(gridDiv, gridOptions);
...
async function visualizarConversa(idConversation, clientName) {
	...
	await $.ajax({
		url: routePegarConversationManutencao,
		type: "POST",
		data: {
	...
	await $.ajax({
		url: routePegarAgendamentoManutencao,
		type: "POST",
		data: {
			idConversation: idConversation,
		},
		...
		success: function (data) {
			if (data.status === 200) {
				...
				let dataHoraManutencao =
					AgendamentoPrincipal["dataHoraManutencao"];
				if (dataHoraManutencao && dataHoraManutencao.includes("|")) {
					dataHoraManutencao = dataHoraManutencao.split("|");
					dataHoraManutencao =
						dataHoraManutencao[dataHoraManutencao.length - 1];
				}
				...
				$("#DataHoraInstalacaoAgendamento").val(
					new Date(dataHoraManutencao).toLocaleString()
				);

				$("#Status").val(AgendamentoPrincipal["status"]);

				let InstaladorAgendamento =
					AgendamentoPrincipal["telefoneMantenedor"];
				...
				$("#MantenedorAgendamento").val(
					AgendamentoPrincipal["nomeMantenedor"]
						? AgendamentoPrincipal["nomeMantenedor"]
						: ""
				);
				$("#TelefoneMantenedorAgendamento").val(
	...
	await atualizarAgGridSmsManutencao(idConversation);
...
function visualizarAuditTrack(
	...
	if (confirmation == 0) {
		...
		atualizarAgGridAuditTrackManutencao({
			idConversation: idConversation,
			idMantenedor: idMantenedor,
		});
	}
}
