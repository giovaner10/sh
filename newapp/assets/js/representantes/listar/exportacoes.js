// EXPORTAÃ‡ÃƒO DA TEBELA
function exportarArquivo(tipo, gridOptions, titulo) {
    var colunas = ['id','nome_completo', 'data_criacao', 'cidade', 'estado', 'pais', 'telefone', 'celular', 'email', 'emailshow'];
    switch (tipo) {
        case "csv":
            fileName = titulo + ".csv";
...
function preencherExportacoes(gridOptions) {
    ...
    opcoes.forEach((opcao) => {
        ...
        div.innerHTML = `
        <img src="${button}" alt="Ãcone" style="width: 15px; margin-left: 10px; cursor: pointer;" title="Exportar no formato ${texto}">
    ...
    `;
        ...
        div.addEventListener("click", function (event) {
            ...
            exportarArquivo(opcao, gridOptions, "Relatório de Representantes");
        });
