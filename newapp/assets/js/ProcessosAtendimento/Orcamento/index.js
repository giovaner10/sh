...
// Ajax
function getOrcamentosAgGrid(callback) {
    let router = Router + '/buscarOrcamentos';
    $.ajax({
        url: router,
        type: 'POST',
        dataType: 'json',
        ...
        success: function(data) {
            if (data && data.status == 200) {
                ...
                showAlert('error', 'Não foi possível listar Orçamentos!');
                AgGrid.gridOptions.api.setRowData([]);
            }
...
function formularioEditarOrcamento(orcamentoId)
{
    ...
    // Modal
    $("#modalOrcamento").load(
        Router + "/formularioOrcamento/" + orcamentoId,
        function(response, status, xhr)
        {
            if (status == "error") {
                // Exibir uma mensagem de erro para o usuário
...
//     // Deleta
//     $.ajax({
//         url: Router + '/excluirOrcamento/' + orcamentoId,
//         type: "POST",
//         dataType: "JSON",
//         beforeSend: function() {
//         },
...
function modalExcluirOrcamento(orcamentoId)
{
    Swal.fire({
        title: "Atenção!",
        text: `Tem certeza que deseja excluir esse Orçamento?`,
        icon: "warning",
...
function criarAgGrid() {
                ...
                cellRenderer: function (options) {
                    ...
                    if (data.id) {
                        if (canEdit == "true") {
                            btnsComPermissao += `
                                <div class="dropdown-item dropdown-item-acoes" style="cursor: pointer;">
                                    <a href="javascript:formularioEditarOrcamento('${data.id}')" style="cursor: pointer; color: black;">Editar</a>
                                </div>
                            `;
                        } 
                        ...
                        if (canDel == "true") {
                            btnsComPermissao += `
                                <div class="dropdown-item dropdown-item-acoes" style="cursor: pointer;">
                                    <a href="javascript:modalExcluirOrcamento('${data.id}')" style="cursor: pointer; color: black;">Excluir</a>
                                </div>
                            `;
                        }
                        return `
                            <div class="dropdown dropdown-table" data-tableId="${tableId}" style="position: relative;">
                                ...
                                <div class="dropdown-menu dropdown-menu-right shadow dropdown-menu-acoes" id="${dropdownId}" style="position:absolute;" aria-labelledby="${buttonId}">
                                    <div class="dropdown-item dropdown-item-acoes" style="cursor: pointer;">
                                        <a href="${BaseURL + 'uploads/processos_atendimento_orcamento/' + data.file}" download="${data.titulo_orcamento}" style="cursor: pointer; color: black;">${lang.baixar}</a>
                                    </div>
                                    ${btnsComPermissao}
                                    <div class="dropdown-item dropdown-item-acoes" style="cursor: pointer;">
                                        <a href="${BaseURL + 'uploads/processos_atendimento_orcamento/' + data.file}" target="blank_" style="cursor: pointer; color: black;">${lang.visualizar}</a>
                                    </div>
                                </div>
                            </div>`;
                    } else {
    ...
    preencherExportacoes(gridOptions, "Relatório de Orçamento", "Orcamento", 'opcoes_exportacao');
}
