function exportarArquivo(tipo, gridOptions, menu, titulo) {
    let colunas = ['id', 'ccid', 'linha', 'operadora', 'serial', 'ultimaAtualizacao', 'cliente', 'ccidAuto', 'prestadora', 'dataCadastro', 'status'];
...
$(document).ready(function() {
    var dropdown = $('#opcoes_exportacao_chips');

    $('#dropdownMenuButtonChips').click(function() {
        dropdown.toggle();
    });

    $(document).click(function(event) {
        if (!dropdown.is(event.target) && event.target.id !== 'dropdownMenuButtonChips') {
            dropdown.hide();
        }
    });
});
...
function preencherExportacoes(gridOptions) {
    const formularioExportacoes = document.getElementById('opcoes_exportacao_chips');
    ...
    `;
        ...
        div.addEventListener('click', function(event) {
            ...
            exportarArquivo(opcao, gridOptions, 'RelatorioListaChips', 'Relatório Lista de Chips');
        });
