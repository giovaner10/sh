function exportarArquivo(tipo, gridOptions, menu, titulo) {
    let colunas = ['urlAcessada', 'qtdAcessos'];
...
$(document).ready(function() {
    var dropdown = $('#opcoes_exportacao_relatorio_acessos');

    $('#dropdownMenuButtonRelatorioAcessos').click(function() {
        dropdown.toggle();
    });

    $(document).click(function(event) {
        if (!dropdown.is(event.target) && event.target.id !== 'dropdownMenuButtonRelatorioAcessos') {
            dropdown.hide();
        }
    });
});
...
function preencherExportacoes(gridOptions) {
    const formularioExportacoes = document.getElementById('opcoes_exportacao_relatorio_acessos');
    ...
    `;
        ...
        div.addEventListener('click', function(event) {
            ...
            exportarArquivo(opcao, gridOptions, 'RelatorioDeAcesso', 'Relatório de Acesso');
        });
