function exportarArquivo(tipo, gridOptions, menu, titulo) {
    let colunas = ['data', 'ultimaConfig', 'placa', 'serial', 'cliente', 'contrato', 'valorDiario', 'valorMensal'];
...
$(document).ready(function() {
    var dropdown = $('#opcoes_exportacao_veiculos');

    $('#dropdownMenuButtonVeiculos').click(function() {
        dropdown.toggle();
    });

    $(document).click(function(event) {
        if (!dropdown.is(event.target) && event.target.id !== 'dropdownMenuButtonVeiculos') {
            dropdown.hide();
        }
    });
});
...
function preencherExportacoes(gridOptions) {
    const formularioExportacoes = document.getElementById('opcoes_exportacao_veiculos');
    ...
    `;
        ...
        div.addEventListener('click', function(event) {
            ...
            exportarArquivo(opcao, gridOptions, 'VeiculosDisponiveis', 'Relatório de Veículos Disponíveis');
        });
