function exportarArquivo(tipo, gridOptions, menu, titulo) {
    let colunas = ['dhenvio_log', 'prefixo', 'placa', 'celular_log', 'msg_sms'];
...
$(document).ready(function() {
    var dropdown = $('#opcoes_exportacao_sms');

    $('#dropdownMenuButtonSms').click(function() {
        dropdown.toggle();
    });

    $(document).click(function(event) {
        if (!dropdown.is(event.target) && event.target.id !== 'dropdownMenuButtonSms') {
            dropdown.hide();
        }
    });
});
...
function preencherExportacoes(gridOptions) {
    const formularioExportacoes = document.getElementById('opcoes_exportacao_sms');
    ...
    `;
        ...
        div.addEventListener('click', function(event) {
            ...
            exportarArquivo(opcao, gridOptions, 'RelatorioLogSms', 'Relatório Log de SMS');
        });
