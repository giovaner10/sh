$(document).ready(function () {
  var dropdown = $("#opcoes_exportacao_grupo");

  $("#dropdownMenuButtonGrupo").click(function () {
    dropdown.toggle();
  });
  ...
  $(document).click(function (event) {
    if (
      ...
      event.target.id !== "dropdownMenuButtonGrupo"
    ) {
      dropdown.hide();
    }
  });
...
function exportarArquivo(tipo, gridOptions, titulo) {
  let colunas = ["codigo", "cliente", "veiculo", "placa", "serial"];
...
function preencherExportacoesGrupo(gridOptions) {
  const formularioExportacoes = document.getElementById("opcoes_exportacao_grupo");
  ...
  opcoes.forEach((opcao) => {
    ...
    div.addEventListener("click", function (event) {
      ...
      exportarArquivo(opcao, gridOptions, "RelatorioGruposVeiculos");
    });
