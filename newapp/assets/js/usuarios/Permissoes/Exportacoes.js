...
function exportarArquivo(tipo, gridOptions, titulo, tabela) {
    ...
    if (tabela == 'Permissoes') {
        colunas = ["id", "descricao", "codPermissao", "modulo", "status"];
    } else if (tabela == 'Cargos') {
        colunas = ["id", "descricao", "status"];
    }
...
$(document).ready(function() {
    var dropdown = $('#opcoes_exportacao_cargo');

    $('#dropdownMenuButtonCargo').click(function() {
        dropdown.toggle();
    });

    $(document).click(function(event) {
        if (!dropdown.is(event.target) && event.target.id !== 'dropdownMenuButtonCargo') {
            dropdown.hide();
        }
    });
});
