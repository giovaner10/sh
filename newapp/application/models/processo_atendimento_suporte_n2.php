...
class Processo_atendimento_suporte_n2 extends CI_Model {
    ...
    public function getSuportes()
    {
	    $query = $this->db->select("c.id, c.titulo_suporte, c.data, a.pasta, a.path, a.file, c.id_arquivo")
            ->where("pasta = 'processos_atendimento_suporte_n2' AND c.status = 'ativo'")
            ->order_by("c.id", "DESC")
            ->join("showtecsystem.arquivos as a", "c.id_arquivo = a.id", "inner")
            ->get("showtecsystem.processos_atendimento_suporte_n2 as c");
	    
	    return $query->result();
	}
    public function getSuporte($id)
	{
	    $query = $this->db->get_where("processos_atendimento_suporte_n2", ["id" => $id]);
	    
	    return $query->result()[0];
	}
    public function adicionarSuporte($suporte, $arquivoNome, $arquivo, $usuarioId)
    {
        $pasta = "processos_atendimento_suporte_n2";
        ...
        if (!$this->db->insert("showtecsystem.processos_atendimento_suporte_n2", $dadosSuporte))
    ...
    public function editarSuporte($suporteId, $suporte, $usuarioId, $arquivo = "", $arquivoNome = "")
    {
        ...
        if (!$this->db->update("showtecsystem.processos_atendimento_suporte_n2",
