...
class Usuario_gestor extends CI_Model {
	...
	public function getDuplicadosGrupos() {
		return $this->db->query("SELECT nome, id_cliente, count(*) as quantidade
			FROM showtecsystem.cadastro_grupo
			WHERE nome = 'MASTER'
	...
	public function add($dados) {
		...
		if ($validacao_usuario == true && $validacao_cpf == true && $validacao_telefone == true) {
			...
			if (isset($cliente->permissoes) && $cliente->permissoes)
				$dados['permissoes'] = ($cliente->permissoes);


				
			if($dados['permissoes'] && $dados['permissoes'] != null && $dados['permissoes'][0] == '['){
				$dados['permissoes'] = stripslashes($dados["permissoes"]);
			} else if ($dados['permissoes'] && $dados['permissoes'] != null && $dados['permissoes'][0] == 'a'){
				$dados['permissoes'] = unserialize($dados['permissoes']);
				$dados['permissoes'] = '["'.implode('","', $dados['permissoes']).'"]';
			} else {
				$dados['permissoes'] = "[]";
			}

			if ($dados['tipo_usuario'] == "monitoramento"){
				$dados['permissoes'] = "[]";
			}
	...
	public function listarCustom($where, $select = 'permissoes') {
		$this->db->select($select);
		
		if (isset($where['tipo_usuario']) && is_array($where['tipo_usuario'])) {
			$this->db->where_in('tipo_usuario', $where['tipo_usuario']);
			unset($where['tipo_usuario']);
		}
	...
	public function atualizar($id_usuario, $dados) {
		...
		if($usuario){
			...
			if (!$erro) {
				...
				if ($dados["tipo_usuario"] == "administrador" || $dados["tipo_usuario"] == "ADMINISTRADOR"){
					$cliente = [];
					if (isset($usuario->id_cliente) && (int) $usuario->id_cliente)
						$cliente = $this->db->select('permissoes')->get_where('showtecsystem.cad_clientes', array('id' => $usuario->id_cliente))->row();
					...
					if(isset($permissoes) && $permissoes != null && $permissoes[0] == '['){
						$dados['permissoes'] = stripslashes($permissoes);
					} else if (isset($permissoes) && $permissoes != null && $permissoes[0] == 'a'){
						$dados['permissoes'] = unserialize($permissoes);
						$dados['permissoes'] = '["'.implode('","', $dados['permissoes']).'"]';
	...
	public function get_gestores($like, $id_cliente, $limit=999999){
		$this->db->select('nome_usuario as text, code as id, usuario');
		if ($like)
			$this->db->like($like);
	...
	*/
	...
	/**
	 * Retorna usuários superiores para uma lista de clientes
	 * No formato [cliente]->[usuario1,usuario2,usuario3...]
	 * @param string $ids_clientes - ids dos clientes separados por virgula
	*/
	public function busca_usuarios_superiores_de_clientes($ids_clientes) {
			$sql = "SELECT id_cliente, GROUP_CONCAT( code SEPARATOR ',' ) as usuarios
				FROM showtecsystem.usuario_gestor
				WHERE id_cliente in ($ids_clientes) and tipo_usuario in ('administrador', 'master')
				GROUP BY id_cliente";
	...
	/**
	 * Atualiza os usuarios do cliente
	 * @param int $id_cliente
	 * @param array $dados
	*/
	public function atualizar_usuarios_pelo_cliente($id_cliente, $dados) {
		if (!empty($dados) && is_array($dados)) {
			return $this->db->where('id_cliente', $id_cliente)
				->where_in('tipo_usuario', ['administrador', 'master'])
				->update('showtecsystem.usuario_gestor', $dados);
		}
	...
	/**
	 * Atualiza usuarios para uma lista de clientes
	 * @param array $ids_clientes
	 * @param array $dados
	*/
	public function atualizar_usuarios_pelos_clientes($ids_clientes, $dados) {
		if (!empty($dados) && is_array($dados)) {
			return $this->db->where_in('id_cliente', $ids_clientes)
				->where_in('tipo_usuario', ['administrador', 'master'])
				->update('showtecsystem.usuario_gestor', $dados);
		}
	...
	/**
	 * Atualiza usuarios em lote
	 * @param array $dados - dados a serem atualizados
	 * @param string $campo - campo a ser usado como referencia
	*/
	public function atualizar_usuarios_lote($dados, $campo = 'code') {
		if (is_array($dados) && !empty($dados)) {
			return @$this->db->update_batch('showtecsystem.usuario_gestor', $dados, $campo);
		}
		return false;
	}
