...
<?php if ($placas): ?>
	...
	<div class="span7" style="float: none; margin-left: auto; margin-right: auto;">
		<h4>OS Manutenção - Contrato <?php echo $id_contrato ?></h4>
	</div>
	...
	<div class="span7" style="float: none; margin-left: auto; margin-right: auto; margin-top: 30px;">
			...
			<form method="post" name="formcontato" id="ContactForm" enctype="multipart/form-data" action="<?php echo site_url('servico/gerar_os_manutencao/'.$id_contrato.'/'.$id_cliente) ?>">
				...
				<div class="span7" id="moldura" style="width: 575px;">
					<div class="titulo_moldura">Dados da Manutenção</div>
...
<!--						  <input required class="input span5"  id="instalador" name="instalador" type="text" autocomplete="on" />-->
				...
				<div class="span7" id="moldura" style="width: 575px;">
					...
					<fieldset id="modulos">
					  <div class="control-group">
						<div name='b' id="teste2" class="col-lg-3">
						  <div name='a' id="teste" class="form-group pegar">
							<input type="text" data-tipo='placa' class="form-control " name="veiculo[placa0]" id="nome-filho-1" placeholder="Placa" data-provide="typeahead" data-source='<?php echo $placas ?>' data-items="20" required>
							<br />
							<select data-tipo='serial' class="js-example-basic-single" id="nome-filho-1" placeholder="Serial" name="veiculo[serial0]" required>
							</select>
							<br />
							<select data-tipo='serial' class="js-example-basic-single" name="serial_retirado" required>
							</select>
				...
				<div class="botoes_resposta" style= "float: right;">
					<a href="#" class="btn" onClick="document.getElementById('ContactForm').reset();return false">Limpar</a>
...
<?php else: ?>
	...
	<div class="alert alert-block">
	  <button type="button" class="close" data-dismiss="alert">&times;</button>
	  <h2>Ops!</h2>
	  <h4>Contrato <?php echo $id_contrato ?> não contém placas cadastradas, favor cadastrar placas ao contrato antes de gerar OS de manutenção!</h4>
	</div>
	...
	<div class="" id="loginModal">
		...
		<div class="modal-body">
			<form id="signup" class="" method="post" action="<?php echo site_url('servico/add_status_instalador')?>">
				...
				<input type="hidden" name="gerar" value="gerar_manutencao" />
				<div class="control-group">
	...
	function appendFilho(node, id) {
		...
		$('.pegar > input').each(function(i, k) {
			if (k.getAttribute('data-tipo') == 'placa') {
				...
				this.setAttribute('name','veiculo[serial'+u+']');
				u++;
			};
		...
		if($('#modulos').length > 0) {
			...
			$(document).on('click', '.quantidade', function(e){
				...
				if (currentVal > oldVal) {
					...
					//elem.setAttribute('data-source',result);
