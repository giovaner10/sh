...
<?php echo form_open_multipart('faturas/enviar_retorno', array('id' => 'form-retorno'), array('path' =>'uploads/retorno'))?>
	<div class="row">
		<a href="<?php echo site_url('faturas/baixa_pendente')?>" class="btn btn-primary" style="background-color: red !important;margin-bottom: 10px;"><i class="fa fa-exclamation-circle" aria-hidden="true"></i> Faturas Pendentes</a>
	</div>
	<div class="row">
		<label for="upload" class="custom-file-upload btn btn-primary"><i class="fa fa-folder-open-o" aria-hidden="true"></i>
		  Escolha Um Arquivo
		</label>
...
<div id="bar" class="progress progress-striped upload">
...
<div id="baixa_automatica" class="modal fade" tabindex="-1"
	role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true"
		...
		if(arquivo.val() == '') {
			...
			$('#form-retorno').ajaxSubmit({
				...
				uploadProgress: function(event, position, total, percentComplete) {
					$('.upload').css('background-color', '#03A9F4');
				...
				complete: function(xhr) {
					...
					if (retorno == '1') {
						$('.result_upload').html('Processando arquivo, por favor aguarde...');
						url = '<?php echo site_url('faturas/ajax_baixa_retorno')?>';
						$('#conteudo').load(url, '',
											function() {
												$('.result_upload').html('');
												$('.upload').css('background-color', null);
												$('.upload').css('width', 0+'%');
											});
					} else {
