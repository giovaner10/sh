<div class="text-title">
    <h3 style="padding: 0 20px; margin-left: 15px;"><?= lang("suporte_n2") ?></h3>
...
<div class="row" style="margin: 15px 0 0 15px;">
    ...
    <div class="col-md-9" id="conteudo">
        <div class="card-conteudo card-suporte" style='margin-bottom: 20px; position: relative;'>
            <h3>
                <b><?= lang("suporte_n2") ?>: </b>
                <div style="display: flex;flex-wrap: wrap; justify-content: space-between; align-items: center;">
                    <div class="dropdown" style="margin-right: 10px;">
                        <button class="btn btn-md btn-primary" id='buttonNovoSuporte' onclick="formularioNovoSuporte()"><i class="fa fa-plus" aria-hidden="true"></i> <?= lang("adicionar_suporte_n2") ?></button>
...
<script type="text/javascript">
    var Router = '<?= site_url('ProcessosAtendimento/suporteN2') ?>';
    var BaseURL = '<?= base_url('') ?>';
    var uploadUrl = "<?= base_url('uploads/processos_atendimento_suporte_n2') ?>";
</script>

<script type="text/javascript" src="<?= versionFile('assets/js/OCR', 'locale.pt-br.js') ?>"></script>
<link rel="stylesheet" type="text/css" href="<?= versionFile('assets/css/newLayout', 'global.css') ?>">
<script type="text/javascript" src="<?= versionFile('assets/js/ProcessosAtendimento/SuporteN2', 'suporte_n2.js') ?>"></script>

<div id="modalSuporte"></div>

<script type="text/javascript">
    let canEdit = "<?= $this->auth->is_allowed_block('edi_suporte_n2_processos_atendimento') ? "true" : "false"; ?>";
    let canDel = "<?= $this->auth->is_allowed_block('del_suporte_n2_processos_atendimento') ? "true" : "false"; ?>";
    let suporteEdicaoExclusaoHtml = '';
    let identificadormodalSuporte = 'Suporte';

    function formularioNovoSuporte() {
        $('#buttonNovoSuporte').html('<i class="fa fa-spin fa-spinner"></i> <?= lang('carregando') ?>').attr('disabled', true);

        $("#modalSuporte").load(
            "<?= site_url('ProcessosAtendimento/SuporteN2/formularioSuporte') ?>",
            function() {
                $('#buttonNovoSuporte').html('<i class="fa fa-plus" aria-hidden="true"></i> <?= lang('adicionar_suporte_n2') ?>').attr('disabled', false);
            });
    }
    ...
    function formularioEditarSuporte(suporteId) {
        ...
        $("#modalSuporte").load(
            "<?= site_url('ProcessosAtendimento/SuporteN2/formularioSuporte') ?>/" + suporteId,
            function() {
                HideLoadingScreen();
    ...
    function excluirSuporte(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= site_url("ProcessosAtendimento/SuporteN2/excluirSuporte") ?>/' + id,
                    type: "POST",
                    dataType: "JSON",
                    beforeSend: function (){
                        ShowLoadingScreen();
                    ...
                    complete: function() {
                        HideLoadingScreen();
                        $('#btnExcluirSuporte').html('<?= lang("excluir") ?>').attr('disabled', false);
                    }
                });
            }
        })
