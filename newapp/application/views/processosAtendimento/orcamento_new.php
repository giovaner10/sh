<div class="text-title">
	<h3 style="padding: 0 20px; margin-left: 15px;"><?= lang('orcamento') ?></h3>
...
<div class="row" style="margin: 15px 0 0 15px;">
    <div class="col-md-12" id="conteudo">
        <div id="card-key-account" class="card-conteudo card-ordem" style='margin-bottom: 20px; position: relative;'>
			<h3>
				<b style="margin-bottom: 5px;"><?= lang('orcamento') ?>: </b>
				<div style="display: flex;flex-wrap: wrap; justify-content: space-between; align-items: center;">
                    <button class="btn btn-primary" id="btnAddOrcamento" onclick="formularioNovoOrcamento()" type="button" style="margin-right: 10px; margin-bottom: 5px;"><i class="fa fa-plus" aria-hidden="true"></i> Cadastrar</button>
...
<script type="text/javascript" src="<?= versionFile('assets/js/ProcessosAtendimento/Orcamento', 'index.js') ?>"></script>
<script type="text/javascript" src="<?= versionFile('assets/js/ProcessosAtendimento/Orcamento', 'exportacoes.js') ?>"></script>

<script>
	var Router = '<?= site_url('ProcessosAtendimento/Orcamento') ?>';
	var BaseURL = '<?= base_url('') ?>';
    var SiteURL = '<?= site_url('') ?>';
    const canEdit = "<?= $this->auth->is_allowed_block('edi_orcamento_processos_atendimento') ? "true" : "false"; ?>";
    const canDel = "<?= $this->auth->is_allowed_block('del_orcamento_processos_atendimento') ? "true" : "false"; ?>";
