...
<div>
    <div class="row-fluid">
        <div class="span9">
            <div class="btn-group">
                <a href="<?php echo site_url('cadastros/apresentacao')?>" class="btn btn-success"><i class="icon-plus icon-white"></i> Nova Apresentação</a>
            </div>
        </div>
    </div>
	...
	<?php foreach ($apresentacoes as $apresentacao){?>
		<tr>
			...
			<td style="text-align: center;">
				<?php 
				$query = $this->db->query("SELECT id_apresentacao, file FROM cad_apresentacao_arquivos
				...
				foreach ($query->result_array() as $row) { 
				    ...
				    if($ext != "ppt" && $ext != "ptx"){
				?>
				<a <?php echo $visualizarImagem;?> href="<?php echo base_url("uploads/apresentacoes/$arquivoImage");?>" class="btn btn-mini btn-default vapresentacao<?=$row['id_apresentacao']?>"  title="Visualizar">
                    <i class="fa fa-eye"></i>
                </a>
                <?php } ?>
                <a <?php echo $visualizarDownload;?> href="<?php echo base_url("uploads/apresentacoes/$arquivoDownload");?>" class="btn btn-mini btn-default"  title="Download">
                    <i class="fa fa-file"></i>
                </a>
                <?php $i++;} ?>
				<a href="<?php echo site_url('cadastros/editar_apresentacao/'.$apresentacao->id)?>" class="btn btn-mini btn-primary" title="Editar">
...
function excluir(id) {
	...
	if (r == true) { 
        var url = "<?= site_url('cadastros/excluir_apresentacao').'/' ?>"+id;
