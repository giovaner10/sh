						...
						<div class="card-body">
							...
							<div>
								<form id="formSenha" method="POST" >
									...
									?>
										<label for="password"><?php echo lang('senha');?></label>
										<input type="password" name="senha" id="senha" class="form-control" placeholder="<?php echo lang('digsenha')?>" required>
									</div>
									<div class="form-group">
										<label for="password_confirmation"><?php echo lang('csenha');?></label>
									...
									<div align="center">
										<button class="btn btn-large btn-primary" style="width: 100%" type="submit" id="redefinir">Redefinir</button>
									</div>
								</form>
								<br>
						    </div>
				...
				$.ajax({
					...
					success: function(response){
						if (response.resultado === "success"){
							...
							window.location.href = '<?php site_url('Homes') ?>'
						}
