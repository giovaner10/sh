...
<ul class="nav nav-tabs">
    <li id="abaTabela" class="nav-item">
        <a id="tab-relatorioTabela" data-toggle="tab" href="" class="nav-link active">Relatório de Instalação em Tabela</a>
    </li>
    <li id="abaGrafico" class="nav-item">
        <a id="tab-relatorioGrafico" data-toggle="tab" href="" class="nav-link">Relatório de Instalação em Gráfico</a>
    </li>
</ul>
...
<div id="divRelatorioGrafico" class="container-fluid my-1" hidden>
    <h2 style="grid-column: span 2; margin-bottom: 30px; font-size: 15px; text-align: center;">Relatório de Instalação</h2>
    <div class="col-sm-12">
...
<script>
    const ctxBar = document.getElementById('graficoTotal');
    ...
    var tabelaRelatorioSms = $('#tabelaRelatorioSms').DataTable({
        ...
        ajax: {
            url: '<?= site_url('Auditoria/Agendamento/listarUltimosCemRelatorioTecnicos') ?>',
            type: 'GET',
            dataType: 'json',
        ...
        dom: 'Bfrtip',
        buttons: [{
                filename: "Relatório de Instalação",
                extend: 'pdfHtml5',
                exportOptions: {
                    columns: ':visible'
                },
                ...
                customize: function(doc, tes) {
                    ...
                    titulo = `Relatório de Instalação - ${date}`;
            ...
            },
            {
                ...
                customize: function(win) {
                    ...
                    titulo = `Relatório de Instalação - Técnicos`;
    ...
    $('#BtnPesquisar').click(function() {
        ...
        if (dataInicial > dataAtual) {
            ...
            $.ajax({
                url: '<?= site_url('Auditoria/Agendamento/listarRelatorioSmsPorNomeTecnicoEData') ?>',
                type: 'POST',
                data: form,
                dataType: 'json',
                success: function(data) {
                    if (data.status === 200) {
                        if ($('#abaGrafico').hasClass('active')) {
                            $('#emptyMessage').hide();
                            atualizaGraficoTotal(data.contagemAcoes);
                            tabelaRelatorioSms.clear().draw();
                            tabelaRelatorioSms.rows.add(data.results).draw();
                        } else {
                            tabelaRelatorioSms.clear().draw();
    ...
    $(document).ready(async function() {
        ...
        let tecnicos = await $.ajax({
            url: '<?= site_url('Auditoria/Agendamento/listarTecnicos') ?>',
            dataType: 'json',
            delay: 1000,
            type: 'GET',
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.status === 200) {
                    gerarGraficoTotal(data.contagemAcoes);
                    $('#emptyMessage').hide();
                    $('#loadingMessage').hide();
                } else {
                    alert(data?.results?.mensagem ? data.results.mensagem : 'Erro ao buscar os dados. Tente novamente.')
