function exportarArquivo(tipo, gridOptions, titulo) {
  let colunas = ["id", "serial", "data_agendamento", "hora_agendamento", "tipo", "situacao"];

  switch (tipo) {
    case "csv":
      fileName = `RelatorioAgendamentoIscas.csv`;
      ...
      break;
    case "excel":
      fileName = `RelatorioAgendamentoIscas.xlsx`;
    ...
    case "pdf":
      ...
      pdfMake.createPdf(definicoesDocumento).download(`RelatorioAgendamentoIscas.pdf`);
      break;
  }
}
...
function preencherExportacoes(gridOptions) {
  ...
  opcoes.forEach((opcao) => {
    ...
    div.addEventListener("click", function (event) {
      ...
      exportarArquivo(opcao, gridOptions, 'Relatório Isca Agendamento');
    });
