...
class Cadastro {
	...
	function coletaDados($id,$tipo) {
		...
		switch ($tipo) {
			...
			case 77:
				$this->DB->selectTab("propostas c, cad_clientes cl", "c.*, cl.nome", "WHERE c.id_cliente = cl.id AND c.id = '$id'");
				break;
			...
			case 9:
				...
				break;
			case 10:
				$this->DB->selectTab("cad_equipamentos b, os_equipamentos a", "*", "WHERE a.id_equipamento=b.id AND a.id='$id'");
				break;
			case 11:
			...
			case 15:
				$this->DB->selectTab("cad_clientes cl, cad_faturas ft ", "*, ft.numero as fatura", " WHERE cl.id = ft.id_cliente AND ft.id = '$id'");
				break;
			case 16:
				$this->DB->selectTab("cad_vendedores vd ", " * ", " WHERE vd.id = '$id'");
				break;
	...
	function cadastrarFILE($REQUEST,$tipo) {
		switch ($tipo) {
			case "1": // UPLOAD
				...
				$this->DB->selecttab( "os","*"," where id = '$ndoc'");
				$dd = $this->DB->fetchArray($this->DB->resultado);
				$this->DB->selecttab( "contratos","*"," where id = '".$dd['id_contrato']."'");
				$dc = $this->DB->fetchArray($this->DB->resultado);
				if ($dc['status'] == 1){
					$this->DB->updateTab(	"contratos", "status = 2", "id = '".$dd['id_contrato']."'");
				}
	...
	function cadastrar($REQUEST,$tipo) {
		...
		switch ($tipo) {
			...
			case 77: 	$nome = $_SESSION['nome_cliente']; 	break;
	...
	function adicionarModulosInstalador($instalador,$modulos) {
		...
		$id_inst = $inst['id'];
		$this->DB->updateTab("cad_equipamentos", "status = 4, id_instalador=$id_inst", "id IN ($modulos)");
	...
	function removerModulosInstalador($instalador,$modulos) {
		...
		$this->DB->updateTab("cad_equipamentos", "status = 2, id_instalador = null", "id IN ($modulos)");
		// Separando os IDs dos modulos
		$ids = explode(',',$modulos);
		$numIds = sizeof($ids);
		for ($i = 0; $i < $numIds; $i++) {
	...
	public static function preparaCampos($REQUEST,$tipo) {
		...
		// Campos de DATA
		$keysData = array("data_nascimento", "primeira_mensalidade", "data_inicial", "data_final", "data_solicitacao", "data_contrato", "data_prestacao");
	...
	function devolverModulos($DB,$LOG,$id,$login,$acao,$os,$modulo,$serial,$tipo,$status,$instalador) {
		...
		$DB->updateTab(	"os_equipamentos", "status = 2", "id_os = '$os' and id_equipamento='$modulo' ");
		//$res = $DB->resultado;
	...
	function instaladoModulos($DB,$LOG,$id,$login,$acao,$os,$modulo,$serial,$tipo,$status,$instalador,$placa,$contrato,$cliente) {
		//Faz o UPDATE
		$DB->updateTab(	"cad_equipamentos", "status = 5, placa='$placa'", "id = '$modulo'");
		$res = $DB->resultado;

		$DB->insertTab("contratos_veiculos (id_contrato,id_cliente,tipo,placa,status,id_ose)", "'$contrato','$cliente','1','$placa','1','$id'");

		$DB->updateTab(	"contratos", "status = 2", "id = '$contrato'");
	...
	function gerar_faturas($REQUEST,$cli,$mes,$ano,$ins1) {
		...
		$numero = $this->get_nextid();//$row_lista['numero'] + 1;
		if (!empty($cli)){
			$query = "SELECT * FROM contratos where id_cliente = {$cli} and status in('1','2')  order by vencimento";
		}else{
			$query = "SELECT * FROM contratos where status in ('1','2') order by id_cliente, vencimento";
		}
		...
		do {
			...
			$descricao       = utf8_decode('Locação de módulos de rastreamento veicular');

			$data_vencimento = $ano.'-'.$mes.'-'.$vencime; //$row_lista['vencimento'];
			$quantidade      = $row_lista['quantidade_veiculos'];
			$mostra_taxa     = $row_lista['mostra_fatura_fat'];
			...
			if ($row_lista['boleto']==1){
				...
				if ($id_cliente != $row_lista['id_cliente']) {
					$taxa_boleto     = 0;
				}
			}
			...
			if ($cliente == $row_lista['id_cliente'] && $vencime == $row_lista['vencimento'])
			{
				...
				$numero          = $this->get_nextid();
				$cliente = $row_lista['id_cliente'];
				$vencime = $row_lista['vencimento'];
			}
			exit($row_lista['primeira_mensalidade']);
			if ($row_lista['primeira_mensalidade'] <= $data_vencimento){
				...
				if ($totalRows_busca==0){
					$this->DB->insertTab("cad_faturas (id_contrato,id_cliente,numero,data_vencimento,status,data_emissao,quantidade,valor_unitario,valor_total,total_fatura,instrucoes1,descricao,taxa_boleto,boleto_vencimento,valor_boleto,mostra_taxa_fat)", "'$id_contrato','$id_cliente','$numero','$data_vencimento','$status','$data_emissao','$quantidade','$valor_unitario','$valor_total','$total_fatura','$instrucoes1','$descricao','$taxa_boleto','$data_vencimento','$total_fatura','$mostra_taxa'");
					//}
					// }
					// verificar adesão

					$data_prestacao  = $row_lista['data_prestacao'];
					$prestacoes      = $row_lista['prestacoes'];
					$valor_prestacao = $row_lista['valor_prestacao'];
					$prestacao_paga  = $row_lista['prestacao_paga'];
					$quantidade      = 1;
					$valor_unitario  = $valor_prestacao;
					$valor_total     = $quantidade * $valor_unitario;
					$total_fatura    = $total_fatura + $valor_total;

					$ultima_prestacao = date("Y-m-d",strtotime(date("Y-m-d", strtotime($data_prestacao)) . " +$prestacoes month"));

					if (date("Y-m-d",  strtotime($data_vencimento)) < date("Y-m-d",  strtotime($ultima_prestacao)) )
					{

						$DataInicial = getdate(strtotime($data_prestacao));
						$DataFinal = getdate(strtotime($data_vencimento));
						$Dif = ($DataFinal[0] - $DataInicial[0]) / 86400;
						$meses = round($Dif/30);

						if ($meses >= 0){
							if (date('d', strtotime($data_prestacao)) != $row_lista['vencimento'] ){
								$numero = $numero + 1;
								$data_vencimento = date("Y-m-d",strtotime(date("Y-m-d", strtotime($data_prestacao)) . " +$meses month"));
								$total_fatura    = $valor_total;
							}

							$prestacao_paga = $meses + 1;
							$descricao       = utf8_decode('Valor referente à taxa de adesão '.$prestacao_paga.' / '.$prestacoes);
							$this->DB->insertTab("cad_faturas (id_contrato,id_cliente,numero,data_vencimento,status,data_emissao,quantidade,valor_unitario,valor_total,total_fatura,instrucoes1,descricao,taxa_boleto,boleto_vencimento,valor_boleto,mostra_taxa_fat)", "'$id_contrato','$id_cliente','$numero','$data_vencimento','$status','$data_emissao','$quantidade','$valor_unitario','$valor_total','$total_fatura','$instrucoes1','$descricao','$taxa_boleto','$data_vencimento','$total_fatura','$mostra'");

							$this->DB->updateTab("contratos "," prestacao_paga='$prestacao_paga' "," id='$id_contrato' ");
							//}

							$this->DB->updateTab("cad_faturas "," total_fatura='$total_fatura', valor_boleto='$total_fatura' "," numero = '$numero' ");
		...
		//select sum(valor_total) as valor from cad_faturas where numero=221;# Registros: 1

		//update cad_faturas b set valor_boleto = @valor   where numero=221# 2 rows affected.

		$queryB = "SELECT sum(valor_total) valor from cad_faturas WHERE numero = {$numero}";
		$execq = mysql_query($queryB, $DB_) or die(mysql_error());
		$valor = mysql_fetch_assoc($execq);
		$query = "update cad_faturas set valor_boleto = '".$valor['valor']."' where numero = $numero";
		$execq2 = mysql_query($query, $DB_) or die(mysql_error());

		mysql_free_result($lista);
		return (true);
	}

	function gerar_faturas_diversas($REQUEST,$id, $des, $qtd, $vlr, $cnt, $nro) {
		include 'cfg.php';
		mysql_select_db($dbs, $DB_);
		$query = "SELECT * FROM cad_faturas WHERE numero=$nro";
		$lista = mysql_query($query, $DB_) or die(mysql_error());
		$row_lista = mysql_fetch_assoc($lista);

		$id_contrato     = $cnt;

		$descricao       = utf8_decode($des);
		$id_cliente      = $id;
		$data_vencimento = $row_lista['data_vencimento'];

		$quantidade      = $qtd;
		$valor_unitario  = $vlr;
		$valor_total     = $qtd * $vlr;
		$total_fatura    = $row_lista['total_fatura'] + $valor_total;
		$taxa_boleto     = $row_lista['taxa_boleto'];

		$data_emissao    = $row_lista['data_emissao'];
		$data_pagto = '';
		$valor_pago = '';
		$cod_barras = '';
		...
		$this->DB->insertTab("cad_faturas (id_contrato,id_cliente,numero,data_vencimento,status,data_emissao,quantidade,valor_unitario,valor_total,total_fatura,descricao,taxa_boleto,boleto_vencimento,valor_boleto)", "'$id_contrato','$id_cliente','$numero','$data_vencimento','$status','$data_emissao','$quantidade','$valor_unitario','$valor_total','$total_fatura','$descricao','$taxa_boleto','$data_vencimento','$total_fatura'");
		$this->DB->updateTab("cad_faturas "," total_fatura='$total_fatura', valor_boleto='$total_fatura' "," numero='$numero' ");
	...
	function taxa($nfat=0){
		$this->DB->updateTab(	"cad_faturas", "taxa_boleto=0 ", "numero = '$nfat'");
		$res = $this->DB->resultado;
		$titAcao = "RETIROU TAXA DA FATURA";
		if ($res === TRUE)	// Registra a ACAO no LOG
			$this->LOG->insertLog($this->DB,'O usuário "'.$this->login.'" '. $titAcao .' [ '.$nfat.' ] ');
		return $res;
	...
	/*
	 * metodo para gerar numero da fatura na tabela next_id
	* Criado por Isaias FIlho para resolver problema de duplicação
	*/
	private function get_nextid(){
		mysql_select_db($dbs, $DB_);
		mysql_query('LOCK TABLES next_id WRITE;');
		$query = mysql_query("SELECT next_id FROM next_id WHERE id = 1");
		$result = mysql_fetch_array($query);
		$numero = $result['next_id'] +1;
		$query_up = mysql_query("UPDATE next_id SET next_id = {$numero} WHERE id = 1");
	 ...
	 * NOVO GERAR FATURAS
	*/
	public function novo_gerar_faturas($REQUEST,$cli,$mes,$ano,$ins1){
		
		$contratos = $this->get_contrato($cli);
		if(count($contratos) > 0){
			/*
			 * organiza contrato por cliente
			 */
			foreach($contratos as $key => $valor){
				$contrs_cliente[$key][] = $contrato;
			}
		...
		$dia_vencimento = $mes == 2 && $contrato->vencimento > 28 ? 28 : $contrato->vencimento;
		$dt_vencimento = $ano.'-'.$mes.'-'.$dia_vencimento;
		/*
	...
	/*
	 * retorna array de objetos contratos por cliente
	*/
	private function get_contrato($cliente){
		$this->DB->selectTab('contratos', '*', "WHERE id_cliente = {$cliente}
							AND status in('1','2') ORDER BY vencimento");

		if($this->DB->numRows($this->DB->resultado) > 0){
			$contrato = $this->DB->fetchArray($this->DB->resultado);
	...
	private function get_faturas_contrato($contrato, $vencimento){
		$this->DB->selectTab('cad_faturas', '*', "WHERE id_contrato = {$contrato} AND 
							data_vencimento = '{$vencimento}'");
		if($this->DB->numRows($this->DB->resultado) > 0){
			$faturas = $this->DB->fetchArray($this->DB->resultado);
			return $faturas;	
		}
