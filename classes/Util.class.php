...
class Util {
	...
	public static function estados() {
        ...
        public static function status_contrato() {
        ...
        public static function status_proposta() {
            $status_contrato = array(
                "0"=>"Cadastrado", 
                "1"=>"Ativo",
                "2"=>"Cancelado",
        ...
        public static function status_cliente() {
        ...
        public static function tipo_proposta() {
            $tipo_proposta = array(
                "0"=>"GESTOR - Rastreador", 
                "1"=>"Ponto Eletronico do Motorista"
	...
	public static function formataData($data){
        ...
        public static function diferencaData($data){
                $databd=$data; // coloque a data vinda do banco de dados
	...
	public static function arraySelectVendedores($DB,$id = ""){
		...
		while ($d = $DB->fetchArray($DB->resultado)) {
			$selected = ($d['id'] == $id)?1:0;
			$arrSel[] = array($d['id'],utf8_decode($d['nome']),$selected);
		}
	...
	/**
	 * Metodo que monta o ARRAY para ser utilizada pela
	 * classe HTML na montagem do SELECT de Fornecedore
	 * @param $DB - Objeto com a conexao ao Banco de Dados
	 * @param $id - ID do fornecedor, caso exista, para deixa-lo selecionado no SELECT
	 * @return ARRAY resultante
	 */
	public static function arraySelectFornecedores($DB,$id = ""){
	
		$DB->selectTab("cad_fornecedores", "id, nome", " ORDER BY nome ASC");
		$num = $DB->numRows($DB->resultado);
	...
	public static function arraySelectEquipamentos($DB,$id = ""){
		...
		while ($d = $DB->fetchArray($DB->resultado)) {
			$selected = ($d['id'] == $id)?1:0;
			$arrSel[] = array($d['id'],utf8_decode($d['modulo']),$selected);
		}
	...
	public static function simnao() {
        ...
        public static function corrigir_valor($valor=0,$dias=0,$juros=8,$multa=2){
                $juros =  ((($valor * $juros)/100)/30) * $dias;
                $multa = (($multa * $valor) / 100);
                return ($valor+$juros+$multa);
        }
        
        public static function data_for_humans($data){
                $dt = explode("-", str_replace('/', '-', $data));
                return $dt[2].'/'.$dt[1].'/'.$dt[0];
        }
        
        public static function dh_for_humans($data, $hora = true, $remove_data = false){
	$dt = explode("-", $data);
	$hr = explode(" ", $dt[2]);
