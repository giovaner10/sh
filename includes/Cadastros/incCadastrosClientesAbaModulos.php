<?php
	...
	$T = new Template(substr($template_dir,3)."/cadastros_equipamentos.html");
	...
	$arrParametros = array ( "Grid_Id"        => "gridEquipamentos", 
							 ...
							 "Grid_Campos"    => "e.id,e.serial,e.modelo,e.marca,CASE e.status WHEN 0 THEN \'Bloqueado\' WHEN 1 THEN \'Cadastrado\' WHEN 2 THEN \'Em teste\' WHEN 3 THEN \'Em transito - OS\' WHEN 4 THEN \'Em transito - Instalador\' WHEN 5 THEN \'Em uso\' WHEN 6 THEN \'Em manutenção\' END AS status, c.ccid AS chip,e.placa", 
							 "Grid_Condicao"  => "e.status=5 AND e.id_cliente=".$_SESSION['passa_id'], 
							 "Grid_SortName"  => "id", 
							 "Grid_SortOrder" => "asc",
							 "Grid_Linhas"	  => "15",
							 "Grid_Width" 	  => "630",
