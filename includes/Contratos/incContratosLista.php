<?php
	...
	$arrParametros = array ( "Grid_Id"        => "gridContratos", 
							 ...
							 "Grid_Campos"    => "c.id,cl.nome,c.quantidade_veiculos,date_format(c.data_cadastro, \'%d/%m/%Y %Hh%i\') AS data,CASE c.status WHEN 0 THEN \'Cadastrado\' WHEN 1 THEN \'Em Transito OS\' WHEN 2 THEN \'Ativo\' WHEN 3 THEN \'Cancelado\' WHEN 4 THEN \'Em Teste\' WHEN 5 THEN \'Bloqueado\' WHEN 6 THEN \'Encerrado\'END AS status", 
							 "Grid_Condicao"  => "c.id_cliente = cl.id", 
							 "Grid_SortName"  => "id", 
							 "Grid_SortOrder" => "asc",
							 "Grid_Linhas"	  => "15",
							 "Grid_Width" 	  => "670",
	...
	$arrColunas[] = array ( "Status", "status", "80", "centar" );
	
	$arrBusca[] = array ( "Contrato", "c.id", "true" );
	$arrBusca[] = array ( "Cliente", "cl.nome", "false" );

	$arrBotoes = array("Cadastrar","Editar","Imprimir");
