<?php
	...
	$T -> Set("OS_Titulo","Contratos NOVOS");
	$T -> Show("OS_Titulo");
	...
	$arrParametros = array ( "Grid_Id"        => "gridContratos", 
							 "Grid_Titulo"    => "Lista de Contratos NOVOS", 
							 "Grid_Tabela"    => "contratos c, cad_clientes cl", 
							 //"Grid_Campos"    => "c.id,cl.nome,(c.quantidade_veiculos+'/'(select count(placa) from contratos_veiculos where id_contrato=c.id)) as instalado, date_format(c.data_cadastro, \'%d/%m/%Y %Hh%i\') AS data,CASE c.status WHEN 0 THEN \'Cadastrado\' WHEN 1 THEN \'OS\' WHEN 2 THEN \'Ativo\' WHEN 3 THEN \'Cancelado\' END AS status", 
                                                         "Grid_Campos"    => "c.id,cl.nome,c.quantidade_veiculos, (select count(placa) from contratos_veiculos where id_contrato=c.id) as instalado, date_format(c.data_cadastro, \'%d/%m/%Y %Hh%i\') AS data,CASE c.status WHEN 0 THEN \'Cadastrado\' WHEN 1 THEN \'OS\' WHEN 2 THEN \'Ativo\' WHEN 3 THEN \'Cancelado\' END AS status", 
							 "Grid_Condicao"  => "c.id_cliente = cl.id AND c.status IN (0) ", //c.status = 0", 
							 "Grid_SortName"  => "c.id", 
							 "Grid_SortOrder" => "asc",
							 "Grid_Linhas"	  => "15",
							 "Grid_Width" 	  => "700",
	...
	$arrColunas[] = array ( "Ve&iacute;culos", "c.quantidades_veiculos", "60", "center" );
        $arrColunas[] = array ( "Instalado", "instalado", "60", "center" );
