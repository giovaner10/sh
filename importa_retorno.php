			...
			if(move_uploaded_file($tmp, $pasta."/".$nome)){
				...
				while (!feof($lendo)){
					...
					if($t_u_segmento == 'U'){
	                    ...
	                    $insert = "UPDATE cad_faturas SET valor_pago='$u_v_pago', data_pagto='$u_dt_ocorencia', valor_pago_fn='$u_v_pago', pagamento_fn='$u_dt_ocorencia', status_fn='Pago', data_fn='$data_agora', hora_fn='$hora_agora', retorno_fn='$nome', status='1' WHERE numero='$numero'";
						$query = mysql_query($insert);
	                    
	                    $query_s = "select * from cad_clientes a, cad_faturas b where a.id=b.id_cliente and b.numero='$numero'";
	                    $lista = mysql_query($query_s, $conexao) or die(mysql_error());
	                    $row_lista = mysql_fetch_assoc($lista);
	                    
	                    echo '<tr>';
	                    echo '<td align="left">'.  Util::zeros(3, $ii).'</td>';
	                    echo '<td align="left">'.  Util::zeros(10, $numero).'</td>';
	                    echo '<td align="left">'.  Util::formataData($row_lista['boleto_vencimento']).'</td>';
	                    echo '<td align="left">'.  Util::formataData($u_dt_ocorencia).'</td>';
	                    echo '<td align="right">'.  Util::formataValor($u_v_pago).'</td>';
